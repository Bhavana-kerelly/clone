...
export default function GatedDevelopment() {
  ...
  // Micro-Mouse Parallax Tracking for Architectural Row Cards
  const handleMouseMove = (e, currentTarget) => {
    const rect = currentTarget.getBoundingClientRect();
    const x = e.clientX - rect.left - rect.width / 2;
    const y = e.clientY - rect.top - rect.height / 2;
    
    gsap.to(currentTarget.querySelector('.media-shutter'), {
      x: x * 0.05,
      y: y * 0.05,
      duration: 0.4,
      ease: 'power2.out'
  ...
  const handleMouseLeave = (currentTarget) => {
    gsap.to(currentTarget.querySelector('.media-shutter'), { x: 0, y: 0, duration: 0.6 });
  };
    ...
    <div ref={containerRef} className="bg-cream min-h-screen text-dark-blue selection:bg-[#c4a468] selection:text-white">
      ...
      <section className="relative h-[70vh] min-h-[500px] flex items-center justify-center px-6 md:px-12 bg-dark-blue text-white overflow-hidden">
        ...
        <div ref={heroRef} className="relative z-10 max-w-5xl mx-auto text-center flex flex-col items-center">
          <span className="text-[11px] font-bold tyranny-text tracking-[0.4em] uppercase text-[#c4a468] mb-4">
            Luxury Communities
          </span>
      ...
      <section className="py-28 border-t border-dark-blue/5 bg-gradient-to-b from-[#2e415703] to-cream px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <div ref={listingsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-16">
          {developmentProperties.map(p => (
            ...
            >
              {/* Media Container Layer Frame */}
              <div className="media-shutter relative aspect-[4/5] w-full overflow-hidden rounded-xl bg-dark-blue mb-6 shadow-[0_20px_50px_rgba(0,0,0,0.08)] transition-transform duration-300 ease-out">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-dark-blue/60 via-transparent to-transparent opacity-50 z-10" />
              </div>
